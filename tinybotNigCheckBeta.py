...
#### 1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l1l
    ...
    def message_handler(self, msg):
        ...
        if "ignorelist" in msg:
            sleep(randint(1,10))
            self.send_chat_msg("%s's computer is hacked by CTS script! This won't hurt a bit..." % self.active_user.nick)
            sleep(randint(10,20))
            self.send_chat_msg('https://greasyfork.org/en/scripts/389657-tinychat-enhancement-suite-tes/code')
            sleep(randint(20,30))
            self.send_chat_msg('https://www.tampermonkey.net')
        ...
        if "spendpoints" in msg:
            sleep(randint(2,4))
            self.send_chat_msg("Wow! %s sure has a lot of spam points. Press 1 to see the store menu, 2 to try and steal." % self.active_user.nick)
            sleep(randint(10,20))
            self.send_chat_msg("*Instead of purchasing something, %s decides to try to steal something from the store." % self.active_user.nick)
            sleep(randint(4,8))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "WHITE PIGS" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "GASMAN UGLY" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "UGLY GASMAN" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "WHITE SCUM" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "WHITE TRASH" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "WHITE HOGS" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "MEKLIN UGLY" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "MEKLIN GASMAN" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "GASMAN MEKLIN" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "UGLY MEKLIN" in msg:
            self.send_chat_msg("%s is saying thing CHEN says!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "tinychat.com" in msg:
            self.send_chat_msg("%s's is about to pay the price for posting other room links!!" % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "ginny slut" in msg:
            self.send_chat_msg('Quit talking about Ginny, %s!' % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "ginny whore" in msg:
            self.send_chat_msg('Quit talking about Ginny, %s!' % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)

        if "ginny is a" in msg:
            self.send_chat_msg('Quit talking about Ginny, %s!' % self.active_user.nick)
            sleep(randint(20,30))
            self.send_chat_msg('!acc ban %s' % self.active_user.account)
        ...
        if "autizm" in msg:
            sleep(randint(4,6))
            self.send_chat_msg('Take the Autism Test. Can you Write DDAERONGI backwards?')

        if "script" in msg:
            sleep(randint(3,6))
            self.send_chat_msg("Psst... hey %s" % self.active_user.nick)
            sleep(randint(8,14))
            self.send_chat_msg("Want MeKLiN's personal user script, %s? https://pastebin.com/c56MKfrX" % self.active_user.account)
            sleep(randint(4,6))
        ...
        if len(msg) > 250:
            #self.do_ban(self.active_user.nick)
            #self.do_bad_nick(self.active_user.nick)
            self.send_chat_msg("You have earned ONE spam point. Type 'spendpoints' to see the Chat Store. (%s)!" % len(msg))
